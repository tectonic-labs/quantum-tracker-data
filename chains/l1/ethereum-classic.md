...
Ethereum Classic secures consensus with Etchash proof-of-work. Proof-of-work is hash-based, so it is not exposed to Shor's algorithm the way elliptic-curve signature schemes are — the consensus layer is quantum-resistant by construction and requires no migration.

**Current state.** Etchash proof-of-work; no elliptic-curve dependency in the consensus mechanism.
