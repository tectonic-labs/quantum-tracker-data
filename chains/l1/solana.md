...
Solana's transport layer uses [QUIC over UDP with TLS 1.3](https://www.helius.dev/blog/all-you-need-to-know-about-solana-and-quic) (rustls / quinn stack), adopted in Agave 1.13.4. Node identity is Ed25519, and the [gossip protocol](https://blog.syndica.io/sig-engineering-1-gossip-protocol/) is a custom UDP-based scheme in which ContactInfo records are signed with Ed25519. Searches across Firedancer, Agave, and Sig found no PQ-related PRs in the P2P modules; the Foundation's April 27 page does not mention QUIC, TLS, gossip, or node identity.

**Current state.** Ed25519 node identity, TLS 1.3 over QUIC; no PQ alternatives drafted.

**Planned future work.** No SIMD, working-group thread, or specification is currently published for PQ P2P transport on Solana.
...
**Planned future work.** None published. Moving this rating would require a fork-specific commitment to deprecate Ed25519, secp256k1, or secp256r1 acceptance; no such proposal exists.
