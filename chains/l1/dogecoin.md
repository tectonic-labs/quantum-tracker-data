...
Dogecoin secures consensus with Scrypt proof-of-work (merge-mined with Litecoin). Proof-of-work is hash-based, so it is not exposed to Shor's algorithm the way elliptic-curve signature schemes are — the consensus layer is quantum-resistant by construction and requires no migration.

**Current state.** Scrypt proof-of-work; no elliptic-curve dependency in the consensus mechanism.
