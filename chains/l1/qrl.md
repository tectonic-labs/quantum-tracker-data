...
QRL is the oldest purpose-built post-quantum blockchain, launched on mainnet in June 2018 with **XMSS** (eXtended Merkle Signature Scheme) as the sole signature scheme — a stateful hash-based algorithm standardized by NIST in SP 800-208 and RFC 8391. No elliptic-curve cryptography has ever been used at any protocol layer. The codebase has been audited by red4sec and x41 D-sec, and the chain has operated for nearly eight years without a security hotfix. [Project Zond](https://www.theqrl.org/) (testnet live Q4 2025) extends QRL with an EVM-compatible VM, proof-of-stake consensus using PQC validator keys, and on-chain PQC signature verification via precompiles.
...
> **Testnet activity**: Zond Testnet V2 is running **ML-DSA-87** on-chain signature verification precompiles as of March 31, 2026. This is not yet active on mainnet.
...
**Current state.** Ephemeral Messaging is live and PQC-native from genesis.

**Planned future work.** Zond adds enhanced messaging capabilities with **SPHINCS+** support and potential smart-contract-based messaging protocols.
